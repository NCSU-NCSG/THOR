...
!  Parameters:
!
!    Input, real ( kind = d_t ) A(1:3), B(1:3), C(1:3), D(1:3), E(1:3),
!    five 3D points.
!
...
!
!    Input, real ( kind = d_t ) VCL(1:3,1:NPT), the vertex coordinates.
...
!
  do i = 1, 3
    ctr(i) = sum ( vcl(i,vm(1:4)) ) / 4.0_d_t
  end do
...
!    routine; K <= LDA.
!
!    Input, real ( kind = d_t ) A(1:K,1:*), array of points.
!
...
!
!    Input, real ( kind = d_t ) A(1:K,1:*), see routine DHPSRT.
!
...
!
!    Input, real ( kind = d_t ) P(1:K), Q(1:K), two points.
!
...
!
!    Input, real ( kind = d_t ) VCL(1:3,1:*), the vertex coordinate list.
!
...
!  Parameters:
!
!    Input, real ( kind = d_t ) PT(1:3), the 3D point.
!
!    Input, real ( kind = d_t ) CTR(1:3), the 3D point in interior of
!    triangulation.
!
!    Input, real ( kind = d_t ) VCL(1:3,1:*), the vertex coordinate list.
!
...
!  integer ( kind = 4 ) htsrc
  ...
  do
    ...
    if ( 1 <= in ) then
      ...
      do j = 1, 3
        if ( alpha(j) < 0.0_d_t ) then
          kneg = kneg + 1
        else if ( alpha(j) == 0.0_d_t ) then
          kzero = kzero + 1
        end if
      end do
!
...
!
        ...
        if ( alpha(1) < 0.0_d_t ) then
          call i4_swap ( a, c )
        else if ( alpha(2) < 0.0_d_t ) then
          call i4_swap ( b, c )
        end if
...
!    to be made.
!
!    Input, real ( kind = d_t ) A(1:3), B(1:3), C(1:3), D(1:3), vertices
!    of tetrahedron.
!
!    Input, real ( kind = d_t ) E(1:3), a fifth point; referenced iff
!    INTEST is TRUE.
!
!    Output, real ( kind = d_t ) CENTER(1:3), center of sphere; undefined
!    if A,B,C,D coplanar.
!
!    Output, real ( kind = d_t ) RADSQ, the square of radius of sphere;
...
!
  ...
  rhs(1) = 0.5_d_t * sum ( da(1:3)**2 )
  rhs(2) = 0.5_d_t * sum ( db(1:3)**2 )
  rhs(3) = 0.5_d_t * sum ( dc(1:3)**2 )
  ...
  if ( abs ( det ) <= 0.01_d_t * tol * cmax ) then
    radsq = -1.0_d_t
    in = 2
    return
  end if
  ...
  if ( intest ) then
    ...
    if ( ( 1.0_d_t + tol ) * radsq < dsq ) then
      in = -1
    else if ( dsq < ( 1.0_d_t - tol ) * radsq ) then
      in = 1
    else
      in = 0
    end if
...
!  Parameters:
!
!    Input, real ( kind = d_t ) A(1:3), B(1:3), C(1:3), D(1:3), 4 vertices
!    of tetrahedron.
!
!    Input, real ( kind = d_t ) E(1:3), fifth point for which
!    barycentric coordinates found
!
!    Output, real ( kind = d_t ) ALPHA(1:4), the scaled barycentric coordinates
...
!
  ...
  if ( abs ( det ) <= 0.01_d_t * tol * max ( amax, bmax, cmax, dmax ) ) then
    degen = .true.
    return
  end if
...
!    enough.
!
!    Input, real ( kind = d_t ) VCL(1:2,1:*), the coordinates of 2D vertices.
!
...
!  Parameters:
!
!    Input, real ( kind = d_t ) X, Y, the 2D point.
!
...
!  Parameters:
!
!    Input, real ( kind = d_t ) X, Y, the 2D point outside convex hull.
!
!    Input, real ( kind = d_t ) VCL(1:2,1:*), the coordinates of 2D vertices.
!
!    Input, integer ( kind = 4 ) TIL(1:3,1:*), the triangle incidence list.
...
!
  ...
  do
    ...
    lr = lrline(x,y,vcl(1,a),vcl(2,a),vcl(1,b),vcl(2,b),0.0_d_t)
...
!  Parameters:
!
!    Input, real ( kind = d_t ) XU, YU, XV1, YV1, XV2, YV2, vertex coordinates;
...
!
!    Input, real ( kind = d_t ) DV, signed distance (positive for left).
!
...
!  Parameters:
!
!    Input, real ( kind = d_t ) X0, Y0, X1, Y1, X2, Y2, X3, Y3,
!    vertex coordinates.
!
...
!
!    Input, real ( kind = d_t ) VCL(1:K,1:*), K-D vertex coordinate list.
!
!    Input, real ( kind = d_t ) PT(1:K), K-D point for which barycentric
!    coordinates are to be computed.
!
!    Output, real ( kind = d_t ) ALPHA(1:K+1), barycentric coordinates
!    (if DEGEN = .FALSE.) such that
...
!    degenerate simplex.
!
!    Workspace, real ( kind = d_t ) MAT(1:K,1:K), matrix used for solving
...
!
  ...
  if ( .not. degen ) then
    ...
    alpha(k+1) = 1.0_d_t - sum ( alpha(1:k) )
  end if
...
!  Parameters:
!
!    Input/output, real ( kind = d_t ) A(LDA,N).  On input, the N by N matrix
...
!
!    Input, real ( kind = d_t ) TOL, the relative tolerance for detecting
!    singularity of A.
!
...
!  Parameters:
!
!    Input, real ( kind = d_t ) A(1:N,1:N), contains factors L, U output
!    from routine LUFAC.
!
!    Input, integer ( kind = 4 ) LDA, the leading dimension of array A in calling routine.
!
...
!
!    Input/output, real ( kind = d_t ) B(1:N).  On input, the right hand
