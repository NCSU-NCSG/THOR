...
CONTAINS
  ...
  SUBROUTINE ingestGmesh()
    ...
    DO i = 1, element_count
      READ(in_unit, *, IOSTAT=io_status) temp_int_1, temp_int_2, temp_int_1, &
            block_id(i), temp_int_1, temp_int_1, element_list(i,:)
      IF (io_status .NE. 0) &
    ...
    DO i = 1, bc_count
      READ(in_unit, *, IOSTAT=io_status) temp_int_1, temp_int_2, temp_int_1, &
            side_set(i), temp_int_1, temp_int_1, bc_list(i,:)
      IF (io_status .NE. 0) &
            CALL generateErrorMessage(io_status, err_fatal, 'Issue reading $Elements block of input during BC ingestion')
      IF (temp_int_2 .NE. 2) &
            CALL generateErrorMessage(err_code_default, err_fatal, 'Invalid bc face shape (non-tri)')
    END DO
